...
public class DeathMatch extends MiniGame implements Runnable{
		...
		try{
			while(true){
				...
				case REST:
					...
					break;
				case ENTERREADY:
					if(DEATH_MATCH_PLAY_LEVEL == 1)	Broadcaster.broadcastPacket(kusan, new S_NpcChatPacket(datoo, "30이상 51이하 데스매치 입장 대기중입니다 많은참여 바랍니다", 0));
					else	 Broadcaster.broadcastPacket(datoo, new S_NpcChatPacket(kusan, "52이상 데스매치 입장 대기중입니다 많은참여 바랍니다", 0));
				...
				case PLAY:
					...
					break;
				case END:
					if(playerCount == 1)	remainOnlyWinner();
					else	EndMiniGame();
					ClearMiniGame();
					Thread.sleep(3000L);
					break;
	...
	public void StartMiniGame(){
		...
		for(int i = 0; i < getPlayerMemberCount(); i++){
			...
			if (map.isInMap(sx, sy) && map.isPassable(sx, sy)) {
				L1Teleport.teleport(pc, sx, sy, (short) DEATHMATCH_MAPID, 5, true);
			}else{
				L1Teleport.teleport(pc, 32639, 32897, (short) DEATHMATCH_MAPID, 5, true);
			}
	...
	public void sendMessage(){
		...
		case PLAY:
			for(int i = 0; i < getPlayerMemberCount(); i++){
				pc = playmembers.get(i);
				pc.sendPackets(new S_ServerMessage(1274, String.valueOf(playerCount)));
			}
			break;
		}
	}
	...
	private void decreaseHp(int j) {
		...
		for(int i = 0; i < getPlayerMemberCount(); i++){
			pc = playmembers.get(i);
			if(pc.isGhost()) continue;
			
			if(pc.getCurrentHp() <= j){
				pc.beginGhost(pc.getX(), pc.getY(), (short) pc.getMapId(), true);
	...
	public void addPlayMember(L1PcInstance pc) {
		...
		L1Teleport.teleport(pc, 32658, 32899, (short) DEATHMATCH_MAPID, 2, true); // 텔
	}

	@Override
	protected void giveBackAdena(L1PcInstance pc) {
