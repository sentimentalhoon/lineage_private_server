...
public class LastabardData {
	public static int getPos(int mapId, int locX, int locY) {
		...
		if(mapId == 531 && locX == 32757 && locY == 32744) pos = 0;
		if(mapId == 531 && locX == 32791 && locY == 32786) pos = 1;
		if(mapId == 531 && locX == 32845 && locY == 32857) pos = 2;
					
		if(mapId == 533 && locX == 32859 && locY == 32897) pos = 0;
		if(mapId == 533 && locX == 32789 && locY == 32891) pos = 1;
		if(mapId == 533 && locX == 32753 && locY == 32811) pos = 2;
	...
	public static int getDelayTime(int mapId) {
		...
		case 466: case 474: case 493: case 494: case 496:
			...
			break;
		// 5분
		case 530: case 532: case 534: // 케이나, 이데아 ,카산드라 죽은 후 시간 제한
			delayTime = 300; // 60 * 5
			break;
		default:
			delayTime = 1200;
	...
	public static int getPosInMapId(int mobMapId) {
		if(mobMapId == 531) return 0;
		if(mobMapId == 533) return 1;
		return 0;
	}

	public static int relatedTime(int mapId) {
		if(mapId == 531) return 530;
		if(mapId == 533) return 532;
		return 0;
	}
