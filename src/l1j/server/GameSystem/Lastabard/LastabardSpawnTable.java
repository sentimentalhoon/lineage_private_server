...
public class LastabardSpawnTable {
	private static Logger log = Logger.getLogger(LastabardSpawnTable.class.getName());
	...
	public void Init() {
		...
		fillSpawnTable();
		log.config("[Lastabard] SpawnList " + spawnTable.size() + "건 불러옴");
	...
	private void fillSpawnTable() {
		...
		try {
			...
			while (rs.next()) {
				...
				if (npcTemplate == null) {
					log.warning("[Lastabard] missing mob data for id:" + npcTemplateId  + " in npc table");
					continue;
				}
		...
		log.fine("[Lastabard] 총 " + spawnCount + "마리");
	}
	...
	public void spawnMobs(int mapId) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM spawnlist_lastabard WHERE mapid = " + mapId);
			rs = pstm.executeQuery();
