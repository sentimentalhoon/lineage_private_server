...
public class PetMatch {
	...
	private synchronized int getPetMatchStatus(int petMatchNo) {
		...
		if (pc1 == null && pc2 == null) return STATUS_NONE;
	...
	public void startPetMatch(int petMatchNo) {
		try{
			...
			Broadcaster.broadcastPacket(_pet1[petMatchNo], new S_SkillSound(_pet1[petMatchNo].getId(), a));
			Broadcaster.broadcastPacket(_pet2[petMatchNo], new S_SkillSound(_pet2[petMatchNo].getId(), b));
			Thread.sleep(4000);
			if(a > b){
		...
		PetMatchTimer timer = new PetMatchTimer(_pet1[petMatchNo], _pet2[petMatchNo], petMatchNo);
		timer.begin();
	}
	...
	public class PetMatchReadyTimer extends TimerTask {
		private Logger _log = Logger.getLogger(PetMatchReadyTimer.class.getName());
	
		private final int _petMatchNo;
		private final L1PcInstance _pc;
		private final L1PetInstance _pet;
	
		public PetMatchReadyTimer(int petMatchNo, L1PcInstance pc, L1PetInstance pet) {
		...
		public void run() {
			try {
				for (;;) {
					...
					if (_pc.isTeleport()) continue;
					
					if (PetMatch.getInstance().setPetMatchPc(_petMatchNo, _pc, _pet) == PetMatch.STATUS_PLAYING) {
	...
	public class PetMatchTimer extends TimerTask {
		...
		public PetMatchTimer(L1PetInstance pet1, L1PetInstance pet2, int petMatchNo) {
			_pet1 = pet1;
			_pet2 = pet2;
			_petMatchNo = petMatchNo;
		}
			...
			try {
				for (;;) {
					...
					if (_pet1.isDead() || _pet2.isDead()) {
						...
						if (!_pet1.isDead() && _pet2.isDead()) {
							winner = 1;
							Broadcaster.broadcastPacket(_pet1, new S_SkillSound(_pet1.getId(), 6354));
						} else if (_pet1.isDead() && !_pet2.isDead()) {
							winner = 2;
							Broadcaster.broadcastPacket(_pet2, new S_SkillSound(_pet2.getId(), 6354));
						} else {
							winner = 3;
						}
