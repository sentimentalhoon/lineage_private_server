...
public class GhostHouse implements Runnable {
	...
	enum Message { ENTER, WAIT_START, NOT_ENOUGH_STARTMEMBERS, GAMEEND };
	enum Status { ENTER, READY, PLAY, END, REST };

	private static final int GHOSTHOUSE_MAPID = 5140;	
	private static final int LIMIT_MIN_PLAYER_COUNT = 2;	// 게임 시작에 필요한 인원 (본섭 : 2명)
	private static final int LIMIT_MIN_ENTER_PLAYER_COUNT = 5;	// 게임 입장에 필요한 인원 (본섭 : 5명)	
	...
	private L1Location GOAL_LINE = new L1Location(32871, 32830, GHOSTHOUSE_MAPID);
	...
	private void sendMessage(Message msg){
		...
		case GAMEEND:
			for(L1PcInstance pc : getPlayMemberArray()){
				if (pc != null){
					if (isTimeOver)	pc.sendPackets(new S_ServerMessage(1263));
	...
	private void GhostHouseDoorClose(){		
		...
		for (L1Object object : L1World.getInstance().getVisibleObjects(5140).values()) {
			if (object instanceof L1DoorInstance) {
				door = (L1DoorInstance) object;
				if (door.getMapId() == GHOSTHOUSE_MAPID && door.getOpenStatus() == ActionCodes.ACTION_Open) {
					door.close();
				}
			}
		}
	...
	private void sortRankList() {
		...
		for(int i = 0; i < c; i++){
			for(int j = 0; j < (c - i); j++){
				if(rankList[j].getLocation().getLineDistance(GOAL_LINE) > rankList[j + 1].getLocation().getLineDistance(GOAL_LINE)){
					data = rankList[j];
	...
	public void pushFinishLineTrap(L1PcInstance pc){
		if (isPlayingNow()) setStatus(Status.END);
		if (finishMemberCount > winnerCount) return;
	...
	private void giveItemToWinnerMember() {
		...
		for (L1PcInstance pc : finishMember){
			if (_random.nextInt(10) <= 2){
				pc.getInventory().storeItem(L1ItemId.GHOSTHOUSE_WINNER_PIECE, 1);
			}
	...
	public void addEnterMember(L1PcInstance pc) {
		...
		if (!isEnterMember(pc)) {
			...
			if(getStatus() == Status.REST && getEnterMemberCount() >= LIMIT_MIN_ENTER_PLAYER_COUNT){
				setStatus(Status.ENTER);
			}			
		}	
	}
		
	public boolean isReadyNow() { return getStatus() == Status.READY ? true : false;}	
	public boolean isPlayingNow() { return getStatus() == Status.PLAY ? true : false;}
	...
	private void setStatus(Status i) 	{ GhostHouseStatus = i;		}
	private Status getStatus() 			{ return GhostHouseStatus;	}
}