...
public class MySqlCharactersItemStorage extends CharactersItemStorage {
	...
	public void storeItem(int objId, L1ItemInstance item) throws Exception {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO character_items SET id = ?, item_id = ?, char_id = ?, item_name = ?, count = ?, is_equipped = 0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, bless = ?, attr_enchantlvl = ?, end_time = ?, second_id=?, round_id=?, ticket_id=?, regist_level=?, protection=?");
			...
			pstm.setInt(18, item.getRegistLevel());
			pstm.setInt(19, item.getProtection()); 
			pstm.execute();
	...
	public void updateItemRegistLevel(L1ItemInstance item) throws Exception {
	   executeUpdate(item.getId(), "UPDATE character_items SET regist_level = ? WHERE id = ?", item.getRegistLevel());
