...
public class S_DoorPack extends ServerBasePacket {
	...
	*/
	...
	private void buildPacket(L1DoorInstance door) {
		...
		if (door.isDead())										writeC(doorStatus);
		else if (openStatus == ActionCodes.ACTION_Open) 		writeC(openStatus);
		else if (door.getMaxHp() > 1 && doorStatus != 0) 		writeC(doorStatus);
		else 													writeC(openStatus);
