...
public class S_NPCPack extends ServerBasePacket {
	...
	public S_NPCPack(L1NpcInstance npc) {
		...
		if ((npc.getNpcTemplate().is_doppel() && npc.getGfxId().getGfxId() != 31)	// 슬라임의 모습을 하고 있지 않으면 돕펠
				|| npc.getGfxId().getGfxId() == 6632 || npc.getGfxId().getGfxId() == 6634		// 얼음던전
				|| npc.getGfxId().getGfxId() == 6636 || npc.getGfxId().getGfxId() == 6638) {	// 얼음던전
			writeC(4); // 장검
