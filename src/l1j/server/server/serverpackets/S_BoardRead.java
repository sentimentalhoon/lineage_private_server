...
public class S_BoardRead extends ServerBasePacket {
	...
	public S_BoardRead(L1NpcInstance board, int number) {
	  ...
	  } else if (board.getNpcId() == 4200018){
		  buildPacketUser2(board, number);
	...
	private void buildPacketUser2(L1NpcInstance board, int number) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM board_user2 WHERE id=?");
			pstm.setInt(1, number);
			rs = pstm.executeQuery();
			while (rs.next()) {
				writeC(Opcodes.S_OPCODE_BOARDREAD);
	...
	private void buildPacketHunt(L1NpcInstance board, int number) {
		...
		try{
			...
			if(r.next()){
				...
				sb.append("\r\n\r\n");
				sb.append(" 수배자 ->  ").append(r.getString(3)).append("\r\n\r\n");
				sb.append(" 이유 : \r\n\r\n  ").append(r.getString(60)).append("\r\n\r\n");
				writeS(sb.toString());
			}
		}catch(Exception e){
			e.printStackTrace();
