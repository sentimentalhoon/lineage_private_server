...
public class S_Board extends ServerBasePacket {
	...
	public S_Board(L1NpcInstance board) {
		...
		else if (board.getNpcId() == 4200020) 	 buildPacketUser(board, 0);	
		else if (board.getNpcId() == 4200018) 	 buildPacketUser2(board, 0);	
		else if (board.getNpcId() == 4200016) 	 buildPacketHunt(board, 0);
		else
			buildPacket(board, 0);
	}
	...
	private void buildPacketUser(L1NpcInstance board, int number) {
		...
		try {
			...
			id = new int[8];
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM board_user order by id desc");
			rs = pstm.executeQuery();
			while (rs.next() && count < 8) {
				if (rs.getInt("id") <= number || number == 0) {
					id[count] = rs.getInt(1);
	...
	private void buildPacketUser2(L1NpcInstance board, int number) {
		int count = 0;
		String[][] db = null;
		int[] id = null;
		Connection con = null;
		...
		try {
			...
			id = new int[8];
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM board_user2 order by id desc");
			rs = pstm.executeQuery();
			while (rs.next() && count < 8) {
				if (rs.getInt("id") <= number || number == 0) {
					id[count] = rs.getInt(1);
