	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			if (i == 1) {
				...
				L1Teleport.teleport(pc , 32769, 32831, (short) 610, 5, false); // 요정숲
			} else if (i == 3) {
				L1Teleport.teleport(pc, 32644, 32955, (short) 0, 5, false); // 판도라
			} else if (i == 4) {
				L1Teleport.teleport(pc , 34055, 32290, (short) 4, 5, false); // 오렌
			} else if (i == 5) {
				L1Teleport.teleport(pc , 33434, 32809, (short) 4, 5, false); // 기란
			} else if (i == 6) {
				...
				L1Teleport.teleport(pc , 33167, 32775, (short) 4, 5, false); // 켄성 수호탑
			} else if (i == 20) {
				L1Teleport.teleport(pc , 32674, 33408, (short) 4, 5, false); // 윈성 수호탑
			} else if (i == 21) {
				L1Teleport.teleport(pc , 33630, 32677, (short) 4, 5, false); // 기란 수호탑
			} else if (i == 22) {
				L1Teleport.teleport(pc , 33524, 33394, (short) 4, 5, false); // 하이네 수호탑
			} else if (i == 23) {
				...
				L1Teleport.teleport(pc , 32743, 32799, (short) 781, 5, false); // 테베 피라미드 내부
			} else if (i == 28) {
				L1Teleport.teleport(pc , 32735, 32830, (short) 782, 5, false); // 테베 오리시스 제단
			} else {
				L1Location loc = GMCommandsConfig.ROOMS.get(arg.toLowerCase());
				if (loc == null) {
					pc.sendPackets(new S_SystemMessage("1.GMroom 2.상담방 3.판도라 4.오렌 5.기란 6.켄말"));
					pc.sendPackets(new S_SystemMessage("7.윈다 8.하이네 9.은말 10.말섬 11.아덴 12.몽섬"));
