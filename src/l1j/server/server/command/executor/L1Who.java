	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			if (arg.equalsIgnoreCase("전체")) {
				...
				if (gmList.length() > 0) {
					pc.sendPackets(new S_SystemMessage("-- 운영자 (" + countGM + "명)"));
					//pc.sendPackets(new S_SystemMessage(gmList.toString()));
				}
				if (playList.length() > 0) {
					pc.sendPackets(new S_SystemMessage("-- 플레이어 (" + countPlayer + "명)"));
					pc.sendPackets(new S_SystemMessage(playList.toString()));
				}				
				if (shopList.length() > 0) {
					pc.sendPackets(new S_SystemMessage("-- 개인상점 (" + countShop + "명)"));
					pc.sendPackets(new S_SystemMessage(shopList.toString()));
				}
				pc.sendPackets(new S_SystemMessage("-- 허상캐릭 (" + countMonitor + "명)"));
			}
			players = null;
		} catch (Exception e) {
