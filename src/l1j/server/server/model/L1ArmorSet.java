	...
	static {
		...
		for (L1ArmorSets armorSets : ArmorSetTable.getInstance().getAllList()) {
			try {
				...
				impl.addEffect(new DamageEffect(armorSets.getSp(),
						armorSets.getShortHitup(), armorSets.getShortDmgup(),
						armorSets.getLongHitup(), armorSets.getLongDmgup()));
...
class DamageEffect implements L1ArmorSetEffect {
	...
	public DamageEffect(int sp, int shortHitup, int shortDmgup, int longHitup, int longDmgup){
		_sp = sp;
