...
public class BravePotion extends L1ItemInstance{
	...
	public void clickItem(L1Character cha, ClientBasePacket packet){
		if(cha instanceof L1PcInstance){			
			...
			if ((itemId == L1ItemId.POTION_OF_EMOTION_BRAVERY // 치우침 이브 일부
					|| itemId == L1ItemId.B_POTION_OF_EMOTION_BRAVERY  // 축복된 치우침 이브
				...
				useBravePotion(pc, itemId);		
			} else if (itemId == L1ItemId.UGDRA_FRUIT && pc.isIllusionist()) { // 유그드라열매				
				useFruit(pc, itemId);				
			} else {
	...
	private void useBravePotion(L1PcInstance pc, int item_id) {
		...
		case 40068:
			time = 600;
			if (pc.getSkillEffectTimerSet().hasSkillEffect(STATUS_BRAVE)) { // 용기와는 중복 하지 않는다.
				pc.getSkillEffectTimerSet().killSkillEffectTimer(STATUS_BRAVE);
				pc.sendPackets(new S_SkillBrave(pc.getId(), 0, 0));
				Broadcaster.broadcastPacket(pc, new S_SkillBrave(pc.getId(), 0, 0));
				pc.getMoveState().setBraveSpeed(0);
			}
			if (pc.getSkillEffectTimerSet().hasSkillEffect(WIND_WALK)) { // 윈드워크와는 중복 하지 않는다
		...
		case 40733:
			...
			if (pc.getSkillEffectTimerSet().hasSkillEffect(HOLY_WALK)) { // 호-리 워크와는 중복 하지 않는다
				pc.getSkillEffectTimerSet().killSkillEffectTimer(HOLY_WALK);
				pc.sendPackets(new S_SkillBrave(pc.getId(), 0, 0));
				Broadcaster.broadcastPacket(pc, new S_SkillBrave(pc.getId(), 0, 0));
				pc.getMoveState().setBraveSpeed(0);
			}
			if (pc.getSkillEffectTimerSet().hasSkillEffect(MOVING_ACCELERATION)) { // 무빙 악 세레이션과는 중복 하지 않는다
			...
			if (pc.getSkillEffectTimerSet().hasSkillEffect(STATUS_FRUIT)) { // 유그드라열매와는 중복안됨
		...
		case 140068:
			time = 700;
			if (pc.getSkillEffectTimerSet().hasSkillEffect(STATUS_BRAVE)) { // 용기 효과와는 중복 하지 않는다.
				pc.getSkillEffectTimerSet().killSkillEffectTimer(STATUS_BRAVE);
				pc.sendPackets(new S_SkillBrave(pc.getId(), 0, 0));
				Broadcaster.broadcastPacket(pc, new S_SkillBrave(pc.getId(), 0, 0));
				pc.getMoveState().setBraveSpeed(0);
		...
		if (item_id == 40068 || item_id == 140068 || item_id ==550002) { // 엘븐 와퍼
