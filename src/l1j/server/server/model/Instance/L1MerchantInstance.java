...
public class L1MerchantInstance extends L1NpcInstance {
	...
	public void onTalkAction(L1PcInstance player) {
		...
		if (talking != null) {
			...
			case 71168: // 진명황 단테스
				...
				break;
			case 71180: // 제이프
				if (player.get_sex() == 0)	htmlid = "jp1";// 남자					
				else 						htmlid = "jp3";
			case 71198: 
			...
			case 80067: //첩보원(욕망의 동굴)
				if (player.getQuest(). get_step(L1Quest.QUEST_DESIRE) == L1Quest.QUEST_END) {
					...
					htmlid = "minicod07";
						...
						== 1 && player.getGfxId().getTempCharGfx() == 6034) { // 코라프프리스트 변신
					htmlid = "minicod03";
				} else if (player.getQuest(). get_step(L1Quest.QUEST_DESIRE)
			...
			case 81202: //첩보원(그림자의 신전)
				if (player.getQuest(). get_step(L1Quest.QUEST_SHADOWS) == L1Quest.QUEST_END) {
					...
					htmlid = "minitos07";
						...
						&& player.getGfxId().getTempCharGfx() == 6035) { // 렛서데이몬 변신
					htmlid = "minitos03";
				} else if (player.getQuest(). get_step(L1Quest.QUEST_SHADOWS) == 1 
	...
	private void BattleZone(L1PcInstance pc) {
		...
		if(pc.getLevel() < 69){
			pc.sendPackets(new S_SystemMessage("\\fW 70레벨부터 입장 가능합니다.")); 
			return;
		}
