...
public class L1BoardInstance extends L1NpcInstance {
	...
	public void onAction(L1PcInstance player) {
		...
		}else if (this.getNpcTemplate().get_npcId() == 4200013){//운영자 게시판 2
			player.sendPackets(new S_EnchantRanking(this));
		} else if (this.getNpcTemplate().get_npcId() == 4200017){ //위탁 판매 게시판 추가... NPC번호는 본인팩에 맞게
			player.sendPackets(new S_Board2(this));		  
		} else if (this.getNpcTemplate().get_npcId() == 585825){ // 로또 게시판
	...
	public void onAction(L1PcInstance player, int number) {
		if (this.getNpcTemplate().get_npcId() == 4200017){ //위탁 판매 게시판 추가... NPC번호는 본인팩에 맞게
	...
	public void onActionRead(L1PcInstance player, int number) {	
		...
		}else if (this.getNpcTemplate().get_npcId() == 4200013){//운영자게시판
			if (!player.isGm()) {
				player.sendPackets(new S_ServerMessage(1413));
				return;
			} else {
		...
		} else if (this.getNpcTemplate().get_npcId() == 4500302){  // 자유게시판
