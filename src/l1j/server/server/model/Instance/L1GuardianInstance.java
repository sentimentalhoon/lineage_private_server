	...
	public void searchTarget() {
		...
		for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(this)) {
			...
			if (!pc.isInvisble() || getNpcTemplate().is_agrocoi()) { // 인비지체크
				if (!pc.isElf()) { // 요정이아니면
					targetPlayer = pc;
					Broadcaster.wideBroadcastPacket(this, new S_NpcChatPacket(this, "$804", 2)); // 그대여. 목숨이 아까우면 빨리 이곳을 떠날지어다. 이곳은 그대같은 자가 더럽히지 못할 신성한 곳이다.
					break;
				}
			}
		}
			...
			if (attack.calcHit()){
				if (getNpcTemplate().get_npcId() == 70848) { // 엔트
					...
					if(getInventory().checkItem(40499)){	//버포->껍질
						player.sendPackets(new S_ServerMessage(143, "$755",
								"$770" + " (" + getInventory().findItemId(40499).getCount() + ")")); // \f1%0이%1를 주었습니다
						player.getInventory().storeItem(40505, getInventory().findItemId(40499).getCount()); //버섯포자
						...
						if(getInventory().checkItem(40507, 6)){	//	엔트줄기
							if (chance <= 20){
								...
								player.sendPackets(new S_ServerMessage(143, "$755", "$763" + " (" + 6 + ")")); // \f1%0이%1를 주었습니다.
							}
						}else{							
							if(getInventory().checkItem(40506, 1)){
								...
								if (chance <= 40) Broadcaster.broadcastPacket(this, new S_NpcChatPacket(this, "$822", 0)); // ...지금. 가지. 껍질. 없다. 나중에. 다시. 와라.							
							}
						}
					}
				}
				if (getNpcTemplate().get_npcId() == 70850) { // 판
					...
					if(getInventory().checkItem(40519, 5)){
						...
						if (chance <= 40) Broadcaster.broadcastPacket(this, new S_NpcChatPacket(this, "$824", 0)); // 갈기털이 남아나질 않겠다! 좀 있다해!						
					}
				}
