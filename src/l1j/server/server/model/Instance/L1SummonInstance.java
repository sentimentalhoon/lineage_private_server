...
public class L1SummonInstance extends L1NpcInstance {
	...
	public boolean noTarget() {
		...
		if (_currentPetStatus == 3) {
			...
			if (getLocation().getTileLineDistance(_master.getLocation()) > 2) {
		      ...
		      deleteMe();
		      return true;      
		 } else if( _master.getMapId() == 37 || _master.getMapId() == 65 || _master.getMapId() == 67  || _master.getMapId() == 1005  || _master.getMapId() == 1011) {   //**서먼 용방에서 삭제**//                 
		          _master.getPetList().remove(getId());
		          deleteMe();
		          return true; 
		} else {
	...
	public L1SummonInstance(L1NpcInstance target, L1PcInstance master, boolean isCreateZombie) {
		...
		if (target instanceof L1MonsterInstance && ((L1MonsterInstance) target).get_storeDroped() == 1) {
	...
	public synchronized void deleteMe() { // 펫창
		if (this._destroyed) {
			return;
		}
		...
		for (int i = 0; i < aobj.length; i++) {
			if (aobj[i] != this)
				continue;
			this._master.sendPackets(new S_ReturnedStat(12, i * 3, getId(),
					false));
		}
		// _master.getPetList().remove(getId());
	...
	public void onAction(L1PcInstance attacker) {
		...
		if ((CharPosUtil.getZoneType(this) == 1 || CharPosUtil.getZoneType(attacker) == 1) && isExsistMaster()) {
