...
public class L1FollowerInstance extends L1NpcInstance {
	...
	public boolean noTarget() {
		...
		if (_master.isDead() || getLocation(). getTileLineDistance(_master.getLocation()) > 10) {
			setParalyzed(true);
			spawn(getNpcTemplate(). get_npcId(), getX(), getY(), getMoveState().getHeading(), getMapId());
			deleteMe();
			return true;
	...
	public void spawn(int npcId, int X, int Y, int H, short Map) {
		...
		if (l1npc != null) {
			...
			try {
				...
				Constructor<?> _constructor = Class.forName((new StringBuilder()).append("l1j.server.server.model.Instance. ")
