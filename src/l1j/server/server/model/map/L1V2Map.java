...
public class L1V2Map extends L1Map {
	...
	public boolean ismPassable(int x, int y, int heading) {
		...
		case 1:
			return (accessOriginalTile(x, y)&2)>0 && (accessOriginalTile(x, y-1)&1)>0;
		case 2:
			return (accessOriginalTile(x, y)&1)>0;
		case 3:
			return ((accessOriginalTile(x, y+1)&2)>0 && (accessOriginalTile(x, y+1)&1)>0) || ((accessOriginalTile(x, y)&1)>0 && (accessOriginalTile(x+1, y+1)&2)>0);
		case 4:
			return (accessOriginalTile(x, y+1)&2)>0;
		case 5:
			return ((accessOriginalTile(x, y+1)&2)>0 && (accessOriginalTile(x-1, y+1)&1)>0) || ((accessOriginalTile(x-1, y)&1)>0 && (accessOriginalTile(x-1, y+1)&2)>0);
		case 6:
			return (accessOriginalTile(x-1, y)&1) > 0;
		case 7:
			return ((accessOriginalTile(x, y)&2)>0 && (accessOriginalTile(x-1, y-1)&1)>0) || ((accessOriginalTile(x-1, y)&1)>0 && (accessOriginalTile(x-1, y)&2)>0);
		default:break;
		}
		return false;
	}
	...
	public boolean isPassable(int x, int y, int heading) {
		...
		if (tile == 1 || tile == 9 || tile == 65 || tile == 69 || tile == 73 || tile == 21) {
			return false;
		}
		if (0 != (_map[offset(x, y)] & BITFLAG_IS_IMPASSABLE)) {
			return false;
