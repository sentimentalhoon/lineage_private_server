...
public class Beginner {
	...
	public int GiveItem(L1PcInstance pc) {
		...
		try {
			...
			pstm1.setString(1, "A");
			if (pc.isCrown()) {	pstm1.setString(2, "P");
			} else if (pc.isKnight()) {	pstm1.setString(2, "K");
			} else if (pc.isElf()) {	pstm1.setString(2, "E");
			} else if (pc.isWizard()) {	pstm1.setString(2, "W");
			} else if (pc.isDarkelf()) {	pstm1.setString(2, "D");
			} else if (pc.isDragonknight()) {	pstm1.setString(2, "T");
			} else if (pc.isIllusionist()) {	pstm1.setString(2, "B");
			} else {	pstm1.setString(2, "A");	}
			
			rs = pstm1.executeQuery();
			PreparedStatement pstm2 = null;
			while (rs.next()) {
				try {
					pstm2 = con.prepareStatement("INSERT INTO character_items SET id=?, item_id=?, char_id=?, item_name=?, count=?, is_equipped=?, enchantlvl=?, is_id=?, durability=?, charge_count=?, remaining_time=?, last_used=?, bless=?, attr_enchantlvl=?");					
	...
	public void GiveTestItem(int type, L1PcInstance pc) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm1 = con.prepareStatement("SELECT * FROM beginner_testitem WHERE activate IN(?,?)");
			pstm1.setString(1, "A");
			if (type == 0) {	pstm1.setString(2, "P");
			} else if (type == 1) {	pstm1.setString(2, "K");
			} else if (type == 2) {	pstm1.setString(2, "E");
			} else if (type == 3) {	pstm1.setString(2, "W");
			} else if (type == 4) {	pstm1.setString(2, "D");
			} else if (type == 5) {	pstm1.setString(2, "T");
			} else if (type == 6) {	pstm1.setString(2, "B");
			} else {	pstm1.setString(2, "A");	}			
			rs = pstm1.executeQuery();			
			while (rs.next()) {
				try {
					...
					pc.getInventory().storeItem(rs.getInt(2),rs.getInt(3), rs.getInt(4));
					//pc.sendPackets(new S_ServerMessage(403, item.getItem().getNameId()));
				} catch (SQLException e2) {
					_log.log(Level.SEVERE, e2.getLocalizedMessage(), e2);
				}
			}
