...
public class L1PcInventory extends L1Inventory {
	...
	public int checkAddItem(L1ItemInstance item, int count, boolean message) {
		...
		if (itemExist != null && (itemExist.getCount() + count) > MAX_AMOUNT) {
			if (message) {
				getOwner().sendPackets(new S_ServerMessage(166, "소지하고 있는 아데나",
						"20억을 초과하고 있습니다.")); // \f1%0이%4%1%3%2
			}
			return AMOUNT_OVER;
		}
	...
	// 아이템을 장착 탈착시킨다(L1ItemInstance의 변경, 보정치의 설정, character_items의 갱신, 패킷 송신까지 관리)
