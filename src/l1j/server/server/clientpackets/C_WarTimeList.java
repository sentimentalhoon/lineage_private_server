		...
		if (clan != null) {
			...
			if (castle_id != 0) { // 성주 클랜 아이디
				...
				if (l1castle.getWarBaseTime() == 0) {
					pc.sendPackets(new S_ServerMessage(305));// \f1지금은 전쟁 시간을 변경할 수 없습니다.
					return;
				}
				...
				base_cal.set(1997, 0, 1, 17, 0);// 1997/01/01 17:00(을)를 기점으로 하고 있다
				...
				pc.sendPackets(new S_ServerMessage(300));// 다음 공성전을 위한 시간을 지정해 주십시오.
			}
		}
	}
