...
public class C_LeaveClan extends ClientBasePacket {
	...
	public C_LeaveClan(byte abyte0[], LineageClient clientthread) throws Exception {
		super(abyte0);
	...
	private void leaveClanBoss(L1Clan clan, L1PcInstance player) throws Exception {
		...
		if (clan.getAlliance() != null) {
			player.sendPackets(new S_ServerMessage(ServerMessage.CANNOT_BREAK_CLAN_HAVING_ALLIANCE)); 
			return;
		}
	...
	private void leaveClanMember(L1Clan clan, L1PcInstance player) throws Exception {
		...
		for (int i = 0; i < clanMember.length; i++) {
			clanMember[i].sendPackets(new S_ServerMessage(ServerMessage.LEAVE_CLAN, player_name, clan_name)); // \f1%0ÀÌ %1Ç÷¸ÍÀ» Å»ÅðÇß½À´Ï´Ù. 
		}
