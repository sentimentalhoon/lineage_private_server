...
public class C_SoldierGive extends ClientBasePacket {
	...
	public C_SoldierGive(byte abyte0[], LineageClient clientthread) {
		...
		switch(type) {
		case 0:
			if (soldier.getSoldier1() <= 0) { return; } else { count = soldier.getSoldier1(); } break;
		case 1:
			if (soldier.getSoldier2() <= 0) { return; } else { count = soldier.getSoldier2(); } break;
		case 2:
			if (soldier.getSoldier3() <= 0) { return; } else { count = soldier.getSoldier3(); } break;
		case 3:
			if (soldier.getSoldier4() <= 0) { return; } else { count = soldier.getSoldier4(); } break;
		default: break;
		}
