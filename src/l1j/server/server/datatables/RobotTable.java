...
public class RobotTable {
	...
	private static HashMap<Integer, RobotTeleport> _TeleportFishList = new HashMap<Integer, RobotTeleport>();
	...
	private RobotTable() {
		...
		selectRobotFishTeleportList();
		System.out.println("OK! "+ getRobotTeleportList().size() + " / " + getRobotFishTeleportList().size() + " / "+ timer.get() + " ms");	
	}
	...
	public void selectRobotFishTeleportList() { 
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		...
		try {
			...
			RobotTeleport robotFishTeleport = null;
			
			while (rs.next()) {
				robotFishTeleport = new RobotTeleport();	
				robotFishTeleport.id = rs.getInt(1);
				robotFishTeleport.x = rs.getInt(2);
				robotFishTeleport.y = rs.getInt(3);
				robotFishTeleport.mapid = rs.getInt(4);
				robotFishTeleport.heading = rs.getInt(5);
				
				_TeleportFishList.put(robotFishTeleport.id, robotFishTeleport);
			}
		} catch (SQLException e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
		} finally {
	...
	*/
	...
	public static HashMap<Integer, RobotTeleport> getRobotFishTeleportList() {
		return _TeleportFishList;
	}
}
