...
public class BoardTable {
	...
	public void writeTopicUser(L1PcInstance pc, String date, String title, String content) {
		...
		try {
			...
			pstm2 = con.prepareStatement("INSERT INTO board_user SET id=?, name=?, date=?, title=?, content=?");
			pstm2.setInt(1, (count + 1));
			pstm2.setString(2, pc.getName());
			pstm2.setString(3, date);
			pstm2.setString(4, title);
	...
	public void writeTopicUser2(L1PcInstance pc, String date, String title, String content) {
		int count = 0;
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm1 = con.prepareStatement("SELECT * FROM board_user2 ORDER BY id DESC");
			rs = pstm1.executeQuery();
			if (rs.next()) {
				count = rs.getInt("id");
			}

			pstm2 = con.prepareStatement("INSERT INTO board_user2 SET id=?, name=?, date=?, title=?, content=?");
			pstm2.setInt(1, (count + 1));
			pstm2.setString(2, pc.getName());
			pstm2.setString(3, date);
			pstm2.setString(4, title);
