...
public class NpcTable {
	...
	public static void reload() { //npc 리로드추가 다른곳에서 없어서 제가 만든건데 다른분이 똑같이만들어스면 난감 ?ㅋㄷ
	...
	private void loadNpcData() {
		...
		try {
			...
			while (rs.next()) {
				...
				if (family == null) npc.set_family(0);
				else 				npc.set_family(family.intValue());
				
				int agrofamily = rs.getInt("agrofamily");
				if (npc.get_family() == 0 && agrofamily == 1) 	npc.set_agrofamily(0);
				else 											npc.set_agrofamily(agrofamily);
	...
	public L1NpcInstance newNpcInstance(int id) throws ClassNotFoundException,
			IllegalAccessException, InstantiationException,InvocationTargetException, IllegalArgumentException {
