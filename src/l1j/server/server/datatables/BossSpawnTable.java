				...
				if(rs.getString("cycle_type").equals(name)) {
					...
					for (L1Object object : L1World.getInstance().getObject()) {
						if(object instanceof L1NpcInstance){
							...
							if (npcId == 45488 && npc.getNpcTemplate().get_npcId() == 45488 
									&& (npc.getMapId() == 9 || npc.getMapId()==10)
									&& npc.getMapId() != 666){								
								alreadySpawnCount++;					
							}else if (npcId == 45601 && npc.getNpcTemplate().get_npcId() == 45601 
									&& (npc.getMapId() == 11 || npc.getMapId()==12 || npc.getMapId() == 13)
									&& npc.getMapId() != 666){							
								alreadySpawnCount++;
							}else if (npcId == 4037000 && npc.getNpcTemplate().get_npcId() == 4037000
					...
					// 랜덤 위치 선정
					if(rndX != 0 && rndY != 0) {
						L1Location newLocation = L1Location.randomLocation(loc, 0, rnd.nextInt(1) > 0 ? rndX : rndY, true);
					...
					if (npcId == 46142 && npc.getMapId() == 73
