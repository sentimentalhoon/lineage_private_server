...
public class ModelSpawnTable {
	private static Logger _log = Logger.getLogger(ModelSpawnTable.class.getName());
...
//						L1World.getInstance().storeObject(field);
	...
	public void insertmodel(L1PcInstance pc, int type) {
		...
		int y = pc.getY();
		switch(type) {
		case 0: npcid = 4500318; break;
		case 1: npcid = 4500317;
			x += 1;
			y -= 1;
			break;
		default: break;
		...
		try {
			...
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO spawnlist_model SET npcid=?,note=?,locx=?,locy=?,mapid=?");
