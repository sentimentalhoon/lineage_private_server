...
public class LogTradeAddItem {
	...
	public void storeLogTradeAddItem(L1PcInstance pc, L1PcInstance target, L1ItemInstance item, int itembefore, int itemafter, int tradecount) {
		File file = new File("LogDB/TradeAddItem.txt");
		...
		if(!fileex){
			...
			try{
				out = new DataOutputStream(new FileOutputStream("LogDB/TradeAddItem.txt"));
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#                              거래(additem) 로그입니다.                                 #\r\n".getBytes());
				out.write("#                                                            by 가니                     #\r\n".getBytes());
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#시간	아이피	계정	케릭터id	케릭터이름	상대방ip	상대방계정	상대방id	상대방케릭터이름	아이템id	아이템이름	아이템인챈 아이템갯수 아이템갯수(전)	아이템갯수(후)	아이템갯수(차이)	올린갯수#\r\n".getBytes());
				//out.write("\r\n\r\n".getBytes());
				slog = fm + "	"; // 시간
				...
				_log.warning("TradeAddItem log oufutstream error:" + e);
				e.printStackTrace();
			} finally {
				try{
					out.close();
			...
			String fm = formatter.format(time1.getTime());
			try {
				rfile = new RandomAccessFile("LogDB/TradeAddItem.txt", "rw");
				...
				_log.warning("TradeAddItem log randomacess error:" + e);
				e.printStackTrace();
			} finally {
				try{
					rfile.close();
