...
public class LogElfDwarfIn {
	...
	public void storeLogElfDwarfIn(L1PcInstance pc, L1ItemInstance item, int item_count_before, int item_count_after, int item_in_count) {
		File file = new File("LogDB/ElfDwarfIn.txt");
		...
		if(!fileex){
			...
			try{
				out = new DataOutputStream(new FileOutputStream("LogDB/ElfDwarfIn.txt"));
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#                              엘프창고(in) 로그입니다.                                  #\r\n".getBytes());
				out.write("#                                                            by 가니                     #\r\n".getBytes());
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#시간	아이피	계정	케릭터id	케릭터이름	아이템id	아이템이름	아이템인챈	넣기전갯수(인벤)	넣은후갯수(인벤)	갯수차이(후-전)	아이템갯수(최종)	아이템갯수(넣은수)	갯수차이(차이-넣은수)#\r\n".getBytes());
				//out.write("\r\n\r\n".getBytes());
				slog = fm + "	"; // 시간
				...
				_log.warning("ElfDwarfIn log oufutstream error:" + e);
				e.printStackTrace();
			} finally {
				try{
					out.close();
			...
			String fm = formatter.format(time1.getTime());
			try {
				rfile = new RandomAccessFile("LogDB/ElfDwarfIn.txt", "rw");
				...
				_log.warning("ElfDwarfIn log randomacess error:" + e);
				e.printStackTrace();
			} finally {
				try{
					rfile.close();
