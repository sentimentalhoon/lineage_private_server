...
public class LogDwarfIn {
	...
	public void storeLogDwarfIn(L1PcInstance pc, L1ItemInstance item, int item_count_before, int item_count_after, int item_in_count) {
		File file = new File("LogDB/DwarfIn.txt");
		...
		if(!fileex){
			...
			try{
				out = new DataOutputStream(new FileOutputStream("LogDB/DwarfIn.txt"));
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#                              일반창고(in) 로그입니다.                                  #\r\n".getBytes());
				out.write("#                                                            by 가니                     #\r\n".getBytes());
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				...
				_log.warning("dwarfin log oufutstream error:" + e);
				e.printStackTrace();
			} finally {
				try{
					out.close();
			...
			String fm = formatter.format(time1.getTime());
			try {
				rfile = new RandomAccessFile("LogDB/DwarfIn.txt", "rw");
				...
				_log.warning("dwarfin log randomacess error:" + e);
				e.printStackTrace();
			} finally {
				try{
					rfile.close();
