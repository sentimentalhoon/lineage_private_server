...
public class LogClanDwarfOut {
	...
	public void storeLogClanDwarfOut(L1PcInstance pc, L1ItemInstance item, int item_count_before, int item_count_after, int item_in_count) {
		File file = new File("LogDB/ClanDwarfOut.txt");
		...
		if(!fileex){
			...
			try{
				out = new DataOutputStream(new FileOutputStream("LogDB/ClanDwarfOut.txt"));
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#                              클랜창고(out) 로그입니다.                                 #\r\n".getBytes());
				out.write("#                                                            by 가니                     #\r\n".getBytes());
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#시간	아이피	계정	케릭터id	케릭터이름	클랜id	클랜이름	아이템id	아이템이름	아이템인챈	빼기전갯수(창고)	뺀후갯수(창고)	갯수차이(후-전)	아이템갯수(최종)	아이템갯수(뺀수)	갯수차이(차이-넣은수)#\r\n".getBytes());
				//out.write("\r\n\r\n".getBytes());
				slog = fm + "	"; // 시간
				...
				out.write(slog.getBytes());
				slog = pc.getClanid() + "	"; // 클랜id
				out.write(slog.getBytes());
				slog = pc.getClanname() + "	"; // 클랜이름
				out.write(slog.getBytes());
				...
				_log.warning("ClanDwarfOut log oufutstream error:" + e);
				e.printStackTrace();
			} finally {
				try{
					out.close();
			...
			String fm = formatter.format(time1.getTime());
			try {
				rfile = new RandomAccessFile("LogDB/ClanDwarfOut.txt", "rw");
				...
				_log.warning("ClanDwarfOut log randomacess error:" + e);
				e.printStackTrace();
			} finally {
				try{
					rfile.close();
