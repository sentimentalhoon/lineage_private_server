	...
	public void storeLogEnchantSuccess(L1PcInstance pc, L1ItemInstance item, int enchantbefore, int enchantafter, int enchantnum) {
		...
		if(!fileex){
			...
			try{
				...
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#                              인챈트 성공 로그입니다.                                   #\r\n".getBytes());
				out.write("#                                                            by 가니                     #\r\n".getBytes());
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#시간	아이피	계정	케릭터id	케릭터이름	아이템id	아이템이름	아이템갯수	아이템인챈	아이템인챈(전)	아이템인챈(후)	아이템인챈(차이)#\r\n".getBytes());
				//out.write("\r\n\r\n".getBytes());
				ditem = fm + "	"; // 시간
				...
				out.write(ditem.getBytes());
				ditem = item.getCount() + "	"; // 아이템 갯수
				out.write(ditem.getBytes());
				ditem = enchantbefore + "	"; // 아이템 인챈수 전
				out.write(ditem.getBytes());
				ditem = enchantafter + "	"; // 아이템 인챈수 후
				out.write(ditem.getBytes());
				int enchantdiff = enchantafter > enchantbefore ? enchantafter - enchantbefore : enchantbefore - enchantafter;
				ditem = enchantdiff + "	"; // 아이템 인챈수 차이
				out.write(ditem.getBytes());
				ditem = enchantnum + "\r\n"; // 아이템 인챈수
				out.write(ditem.getBytes());
			} catch (Exception e) {
