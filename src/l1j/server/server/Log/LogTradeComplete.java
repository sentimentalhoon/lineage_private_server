	...
	public void storeLogTradeComplete(L1PcInstance pc, L1PcInstance target, L1ItemInstance item, int itembeforetrade, int itembeforeinven, int itemafter, int tradecount) {
		...
		if(!fileex){
			...
			try{
				...
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#                              거래(complate) 로그입니다.                                #\r\n".getBytes());
				out.write("#                                                            by 가니                     #\r\n".getBytes());
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#시간	아이피	계정	케릭터id	케릭터이름	상대방ip	상대방계정	상대방id	상대방케릭터이름	아이템id	아이템이름	아이템인챈 아이템갯수 아이템갯수(거래창올리기전)	아이템갯수(거래전상대방인벤속)	아이템갯수(거래후상대방인벤속)	아이템갯수(거래전-후차이)	거래갯수#\r\n".getBytes());
				//out.write("\r\n\r\n".getBytes());
				slog = fm + "	"; // 시간
				...
				out.write(slog.getBytes());
				slog = itembeforetrade + "	"; // 아이템 갯수(거래전)
				out.write(slog.getBytes());
				slog = itembeforeinven + "	"; // 아이템 갯수(인벤전)
				out.write(slog.getBytes());
