	...
	public void storeLogClanDwarfIn(L1PcInstance pc, L1ItemInstance item, int item_count_before, int item_count_after, int item_in_count) {
		...
		if(!fileex){
			...
			try{
				...
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#                              클랜창고(in) 로그입니다.                                  #\r\n".getBytes());
				out.write("#                                                            by 가니                     #\r\n".getBytes());
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#시간	아이피	계정	케릭터id	케릭터이름	클랜id	클랜이름	아이템id	아이템이름	아이템인챈	넣기전갯수(인벤)	넣은후갯수(인벤)	갯수차이(후-전)	아이템갯수(최종)	아이템갯수(넣은수)	갯수차이(차이-넣은수)#\r\n".getBytes());
				//out.write("\r\n\r\n".getBytes());
				slog = fm + "	"; // 시간
