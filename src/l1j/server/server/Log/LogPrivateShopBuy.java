/*
 * 개인 상점(buy) 소스입니다. 
 * 이 소스는 개인상점에서 물건을 살 경우..즉 개인상점 buy창을 사용했을때 작동합니다.
	...
	public void storeLogPrivateShopBuy(L1PcInstance pc, L1PcInstance target, L1ItemInstance item, int itembefore, int itemafter, int buycount) {
		...
		if(!fileex){
			...
			try{
				...
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#                              개인상점(buy) 로그입니다.                                 #\r\n".getBytes());
				out.write("#                                                            by 가니                     #\r\n".getBytes());
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#시간	아이피	계정	케릭터id	케릭터이름	상점주인ip	상점주인계정	상점주인id	상점주인케릭이름	아이템id	아이템이름	아이템인챈 아이템갯수(최종)	사기전갯수(상점)	산후갯수(상점)	갯수차이(후-전)	산갯수#\r\n".getBytes());
				//out.write("\r\n\r\n".getBytes());
				slog = fm + "	"; // 시간
