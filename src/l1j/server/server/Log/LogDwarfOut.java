...
public class LogDwarfOut {
	...
	public void storeLogDwarfOut(L1PcInstance pc, L1ItemInstance item, int item_count_before, int item_count_after, int item_in_count) {
		File file = new File("LogDB/DwarfOut.txt");
		...
		if(!fileex){
			...
			try{
				out = new DataOutputStream(new FileOutputStream("LogDB/DwarfOut.txt"));
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				out.write("#                              일반창고(out) 로그입니다.                                 #\r\n".getBytes());
				out.write("#                                                            by 가니                     #\r\n".getBytes());
				out.write("#----------------------------------------------------------------------------------------#\r\n".getBytes());
				...
				_log.warning("DwarfOut log oufutstream error:" + e);
				e.printStackTrace();
			} finally {
				try{
					out.close();
			...
			String fm = formatter.format(time1.getTime());
			try {
				rfile = new RandomAccessFile("LogDB/DwarfOut.txt", "rw");
				...
				_log.warning("DwarfOut log randomacess error:" + e);
				e.printStackTrace();
			} finally {
				try{
					rfile.close();
