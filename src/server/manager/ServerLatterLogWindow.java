...
public class ServerLatterLogWindow extends JInternalFrame {
	...
	public void initialize(String windowName, int x, int y, int width, int height, boolean resizable, boolean closable) {
		...
		try {
			...
			pstm = con.prepareStatement("select * from letter where receiver in ('메티스', '미소피아', '루피') ORDER BY item_object_id DESC");
	...
	private class MouseListenner extends MouseAdapter {
		public void mouseClicked(MouseEvent e) {
			if (e.getButton() == 1) {
				...
				if (column < 5) {
					//JOptionPane.showMessageDialog(null, ((JTable)e.getSource()).getValueAt(row, 3), "내용", JOptionPane.INFORMATION_MESSAGE);
					...
					if (jServerLetterSendWindow == null) {
						jServerLetterSendWindow = new ServerLetterSendWindow("편지", 0, 0, 500, 500, true, true);						
						eva.desktop.add(jServerLetterSendWindow, JLayeredPane.DEFAULT_LAYER);
						...
						jServerLetterSendWindow.setLocation(0, 0);
					}
