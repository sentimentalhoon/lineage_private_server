...
public class ServerUserMoniterWindow extends JInternalFrame {
	...
	public void initialize(String windowName, int x, int y, int width, int height, boolean resizable, boolean closable) {
	    ...
	    jJLabel2 = new JLabel("신비한날개깃털");
	    jJLabel3 = new JLabel("무기마법주문서");
	    jJLabel4 = new JLabel("갑옷마법주문서");
	    ...
	    String[] modelColName = { "계정명", "캐릭명", "온라인", "아데나", "신비한날개깃털", "무기마법주문서", "갑옷마법주문서" };
		...
		jJTable.getColumn("신비한날개깃털").setCellRenderer(new LabelRowCellEdior("신비한날개깃털"));
		jJTable.getColumn("무기마법주문서").setCellRenderer(new LabelRowCellEdior("무기마법주문서"));
		jJTable.getColumn("갑옷마법주문서").setCellRenderer(new LabelRowCellEdior("갑옷마법주문서"));
	...
	private class LabelRowCellEdior extends JLabel implements TableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int rowIndex, int vColIndex) {
			...
			if (isSelected) {
				...
				} else if (columnName.equals("신비한날개깃털") && Integer.parseInt(String.valueOf(value).replaceAll(",", "")) > Integer.parseInt(txt_Feather.getText())) {
					setBackground(Color.pink);
					setHorizontalAlignment(JLabel.RIGHT);
				} else if (columnName.equals("무기마법주문서") && Integer.parseInt(String.valueOf(value).replaceAll(",", "")) > Integer.parseInt(txt_WeaponScroll.getText())) {
					setBackground(Color.pink);
					setHorizontalAlignment(JLabel.RIGHT);
				} else if (columnName.equals("갑옷마법주문서") && Integer.parseInt(String.valueOf(value).replaceAll(",", "")) > Integer.parseInt(txt_ArmorScroll.getText())) {
					setBackground(Color.pink);	
					setHorizontalAlignment(JLabel.RIGHT);
				} else {
					setHorizontalAlignment(JLabel.RIGHT);
