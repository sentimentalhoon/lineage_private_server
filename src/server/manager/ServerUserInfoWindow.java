...
public class ServerUserInfoWindow extends JInternalFrame {
	...
	public void initialize(String windowName, int x, int y, int width, int height, boolean resizable, boolean closable) {
		...
		try {		
			...
			lbl_UserCount = new JLabel("접속자수 : 0");		
			lbl_UserCount.setForeground(Color.red);
			lbl_Helper = new JLabel("유저선택후 클릭 >");
						...
						try {
							...
							*/
							...
							if (player != null) {
								...
								JOptionPane.showMessageDialog(null, txt_UserName.getText() + "님은 현재 월드상에 존재하지 않습니다.", "Server Message", JOptionPane.INFORMATION_MESSAGE);
							}
...
//				}
//			});
		    btn_Ban = new JButton("밴");
		    btn_Ban.setToolTipText("선택한 유저를 밴시킵니다.");
		    ...
		    btn_NoChat = new JButton("채금");
		    btn_NoChat.setToolTipText("선택한 유저를 10분간 채금시킵니다.");
		    ...
		    btn_Present = new JButton("선물");
		    btn_Present.setToolTipText("선택한 유저에게 선물을 지급합니다.");
		    btn_Present.addActionListener(new ActionListener() { 
				public void actionPerformed(ActionEvent e) {
					if (eva.isServerStarted) {
						if (!txt_UserName.getText().equalsIgnoreCase("")) {
							jServerPresentWindow = new ServerPresentWindow("선물", 0, 0, 400, 400, false, true);		
		    ...
		    btn_Poly = new JButton("변신");
		    btn_Poly.setToolTipText("선택한 유저를 변신시킵니다.");
		    btn_Poly.addActionListener(new ActionListener() { 
				public void actionPerformed(ActionEvent e) {
					if (eva.isServerStarted) {
						if (!txt_UserName.getText().equalsIgnoreCase("")) {
							if (!txt_UserName.getText().equalsIgnoreCase("")) {
								jServerPolyWindow = new ServerPolyWindow("변신", 0, 0, 400, 400, false, true);		
		    ...
		    btn_AllPresent = new JButton("전체선물");
		    btn_AllPresent.setToolTipText("월드상 모든 유저에게 선물을 지급합니다.");
		    btn_AllPresent.addActionListener(new ActionListener() { 
				public void actionPerformed(ActionEvent e) {
					if (eva.isServerStarted) {						
						jServerPresentWindow = new ServerPresentWindow("전체선물", 0, 0, 400, 400, false, true);		
						eva.jJDesktopPane.add(jServerPresentWindow, 0);	
						
						jServerPresentWindow.setLocation((eva.jJFrame.getContentPane().getSize().width / 2) - (jServerPresentWindow.getContentPane().getSize().width / 2), (eva.jJFrame.getContentPane().getSize().height / 2) - (jServerPresentWindow.getContentPane().getSize().height / 2));
						jServerPresentWindow.txt_UserName.setText("전체유저");						
					} else {
						eva.errorMsg(eva.NoServerStartMSG);
					}
				}
			});
		    btn_AllPoly = new JButton("전체변신");
		    btn_AllPoly.setToolTipText("월드상 모든 유저를 변신시킵니다.");
		    btn_AllPoly.addActionListener(new ActionListener() { 
				public void actionPerformed(ActionEvent e) {
					if (eva.isServerStarted) {
						if (!txt_UserName.getText().equalsIgnoreCase("")) {
							jServerPolyWindow = new ServerPolyWindow("전체변신", 0, 0, 400, 400, false, true);		
							eva.jJDesktopPane.add(jServerPolyWindow, 0);	
							
							jServerPolyWindow.setLocation((eva.jJFrame.getContentPane().getSize().width / 2) - (jServerPolyWindow.getContentPane().getSize().width / 2), (eva.jJFrame.getContentPane().getSize().height / 2) - (jServerPolyWindow.getContentPane().getSize().height / 2));
							jServerPolyWindow.txt_UserName.setText("전체유저");
						} else {
							eva.errorMsg(eva.blankSetUser);
						}
					} else {
		    ...
		    chk_Infomation = new JCheckBox("정보");
		    chk_Infomation.setSelected(true);
		    chk_Infomation.setToolTipText("체크하시면 인벤, 창고, 장비, 계정 정보를 바로 확인 가능!");
			...
			jJTabbedPane1.addTab("유저리스트", pScroll);
			
			String[] model1ColName = { "", "", "", "" };
			String[] model2ColName = { "번호", "명칭", "수량", "인챈", "속성", "단계" };
			String[] model5ColName = { "계정", "아이피", "캐릭터명", "레벨", "혈맹", "접속" };
			...
			jJTabbedPane2.addTab("정보", jJScrollPane1);
			jJTabbedPane2.addTab("인벤", jJScrollPane2);
			jJTabbedPane2.addTab("창고", jJScrollPane3);
			jJTabbedPane2.addTab("장비", jJScrollPane4);
			jJTabbedPane2.addTab("계정", jJScrollPane5);
