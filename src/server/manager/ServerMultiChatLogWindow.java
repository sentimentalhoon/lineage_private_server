...
public class ServerMultiChatLogWindow extends JInternalFrame {
	...
	public void initialize(String windowName, int x, int y, int width, int height, boolean resizable, boolean closable) {
		...
		try {
		    ...
		    jJTabbedPane.addTab("귓말", whisperChatScroll);
		    jJTabbedPane.addTab("혈맹", clanChatScroll);
		    jJTabbedPane.addTab("파티", partyChatScroll);
		    jJTabbedPane.addTab("장사", tradeChatScroll);
		    jJTabbedPane.addTab("창고", wareHouseScroll);
		    jJTabbedPane.addTab("거래", tradeScroll);
		    jJTabbedPane.addTab("인챈", enchantScroll);
		    jJTabbedPane.addTab("감시", observeScroll);
		    jJTabbedPane.addTab("버그", bugScroll);
		    jJTabbedPane.addTab("명령", commandScroll);
				...
				public void mouseClicked(java.awt.event.MouseEvent evt) {
					try {
						...
						synchronized (eva.lock) {
							...
							eva.flush(whisperChatText, "[" + eva.time + "] 귓말", eva.date);
							eva.flush(clanChatText, "[" + eva.time + "] 혈맹", eva.date);
							eva.flush(partyChatText, "[" + eva.time + "] 파티", eva.date);
							eva.flush(wareHouseText, "[" + eva.time + "] 창고", eva.date);
							eva.flush(tradeText, "[" + eva.time + "] 교환", eva.date);
							eva.flush(enchantText, "[" + eva.time + "] 인챈", eva.date);
							eva.flush(observeText, "[" + eva.time + "] 감시", eva.date);
							eva.flush(bugText, "[" + eva.time + "] 버그", eva.date);
							eva.flush(commandText, "[" + eva.time + "] 명령", eva.date);
							sTemp = null;
							eva.date = null;
							eva.time = null;							
						}
	...
	public void savelog() {
		try {
			...
			synchronized (eva.lock) {
				...
				eva.flush(whisperChatText, "[" + eva.time + "] 3.귓말", eva.date);
				eva.flush(clanChatText, "[" + eva.time + "] 4.혈맹", eva.date);
				eva.flush(partyChatText, "[" + eva.time + "] 5.파티", eva.date);
				eva.flush(wareHouseText, "[" + eva.time + "] 6.창고", eva.date);
				eva.flush(tradeText, "[" + eva.time + "] 7.교환", eva.date);
				eva.flush(enchantText, "[" + eva.time + "] 8.인챈", eva.date);
				eva.flush(observeText, "[" + eva.time + "] 9.감시", eva.date);
				eva.flush(bugText, "[" + eva.time + "] 10.버그", eva.date);
				eva.flush(commandText, "[" + eva.time + "] 11.명령", eva.date);
				sTemp = null;
				eva.date = null;
				eva.time = null;							
			}
