...
public class ServerPresentWindow extends JInternalFrame {
	...
	public void initialize(String windowName, int x, int y, int width, int height, boolean resizable, boolean closable) {
		...
		jJLabel1 = new JLabel("캐릭터명");
		jJLabel2 = new JLabel("번호");
		jJLabel3 = new JLabel("명칭");
		jJLabel4 = new JLabel("갯수");
		jJLabel5 = new JLabel("인챈");
		jJLabel6 = new JLabel("속성");
		jJLabel7 = new JLabel("단계");
		jJLabel8 = new JLabel("검색할명칭");
							...
							if (pc.getName().equalsIgnoreCase(txt_UserName.getText()) || txt_UserName.getText().equalsIgnoreCase("전체유저")) {
								if (temp != null) {
									...
									if (pc.getInventory().checkAddItem(item, count) == L1Inventory.OK) {
										...
										//	eva.LogServerAppend(txt_UserName.getText() + "에게 " + temp.getName() + "을(를) 생성해 주었습니다.", "확인바람");
									} else {
										...
										if (createCount > 0) {
											...
											//eva.LogServerAppend(txt_UserName.getText() + "에게 " + temp.getName() + "을(를)" + createCount + "개 생성했습니다.", "확인바람");
										}
									}
								} else if (temp == null) {
									//eva.LogServerAppend("[선물주기 실패] 그런 아이템은 존재하지 않습니다.", "확인바람");
								}
							}
						}
					} catch (Exception ex) {
