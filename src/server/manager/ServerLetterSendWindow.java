...
public class ServerLetterSendWindow extends JInternalFrame {
	...
	public void initialize(String windowName, int x, int y, int width, int height, boolean resizable, boolean closable) {
	    ...
	    jJLabel1 = new JLabel("받는 캐릭명");
	    jJLabel2 = new JLabel("");
	    jJLabel3 = new JLabel("보낼 캐릭명");
	    jJLabel4 = new JLabel("보낼 제목");
	...
	private void dataSend() {		
		try {
			if (txt_To.getText().equals("")) {
				JOptionPane.showMessageDialog(null, "받을 캐릭명을 입력하세요.", "", JOptionPane.WARNING_MESSAGE);
				return;
			} else if (txt_From.getText().equals("")) {
				JOptionPane.showMessageDialog(null, "보낼 캐릭명을 입력하세요.", "", JOptionPane.WARNING_MESSAGE);
				return;
			} else if (txt_Title.getText().equals("")) {
				JOptionPane.showMessageDialog(null, "보낼 제목을 입력하세요.", "", JOptionPane.WARNING_MESSAGE);
				return;
			} else if (txt_SendMsg.getText().equals("")) {
				JOptionPane.showMessageDialog(null, "보낼 내용을 입력하세요.", "", JOptionPane.WARNING_MESSAGE);
				return;
			}
			...
			JOptionPane.showMessageDialog(null, "편지를 보냈습니다.", "", JOptionPane.INFORMATION_MESSAGE);
			setClosed(true);
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null, "편지 보내기 실패.", "", JOptionPane.ERROR_MESSAGE);
		} 
